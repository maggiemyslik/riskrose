...
# create a path from the root to the leaf for the sunburt plot to read 
  mutate(leaf = paste(level1, level2, level3, level4, sep = "/"),
         root3 = paste(level1, level2, level3, sep = "/"),
         root2 = paste(level1, level2, sep = "/"),
         root1 = level1,
...
```{r}
plot = risks |>
  plot_ly(ids = ~leaf, labels = ~level4, parents = ~root3,
               values = ~value, branchvalues = "total", type = 'sunburst')
