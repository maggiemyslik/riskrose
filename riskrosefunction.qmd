...
load_data = function(filepath) {
  ...
  risks = read_csv(filepath, skip = 3)
  colnames(risks) = as.character(unlist(risks[1, ]))
  risks = risks[-1, ]
    
    # rename relevent variables to usable names 
  risks |>  
...
getColorOuter = function(estrisk) {
  ...
  # convert numeric values into a scale from 0 to 1 to standardize risk level ratings 
    ...
    estrisk_numeric = as.numeric(estrisk) 
    estrisk = estrisk_numeric - min(estrisk_numeric, na.rm = TRUE) / (max(estrisk_numeric, 
    na.rm = TRUE) - min(estrisk_numeric, na.rm = TRUE)) 
      
    # convert estrisk numeric value into standardized color system 
    color_matrix = colorRamp(c("#6aa84f", "#ffbf00", "#d61f1f"))(estrisk)
